...
public class ReaderFeedService extends IntentService {
		...
		try {
			...
			for (Article article : articles) {
				long rowId = db.insertWithOnConflict(
						DbHelper.ARTICLE_TABLE_NAME, null,
						DbHelper.articleToContentValues(article),
						SQLiteDatabase.CONFLICT_IGNORE); // we assume articles
															// stay the same -
															// TODO: don't
				if (rowId != -1) {
					newArticles += 1;
					Log.v(TAG, "- Added article " + article.title
							+ " to database on row " + rowId + ".");
					...
					if (preloadedArticles < MAX_ARTICLES_TO_PRELOAD) {
						Intent getImages = new Intent(this,
								ReaderFeedService.class);
						getImages
								.setAction(Constants.GET_ARTICLE_IMAGES_INTENT);
						getImages.putExtra(Constants.ARTICLE_CONTENT_EXTRA,
								article.content); // TODO: is sending content
													// through intent the best
													// way?
						startService(getImages);
						preloadedArticles += 1;
					}
				} else {
					Log.v(TAG, "- Article " + article.title
							+ " already in database.");
				}
			}
	...
	protected void onHandleIntent(Intent intent) {
		if (intent.getAction().equals(Constants.GET_ARTICLES_INTENT)) {
			...
			String articleContent = intent
					.getStringExtra(Constants.ARTICLE_CONTENT_EXTRA);
