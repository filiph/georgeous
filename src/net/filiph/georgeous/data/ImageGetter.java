...
public class ImageGetter implements ImageGetterWithManageSpace {
	...
	/**
	 * Whether or not to load images from web when they are not available
	 * cached. Normally set to true, but can be set to false when caller uses
	 * ImageGetter to cache images (and so doesn't need to show them).
	 */
	...
	private boolean tryCaching(InputStream in, String url) {
		...
		try {
			final File file = new File(mExternalCacheDir,
					getRelativeCacheFilename(url));
