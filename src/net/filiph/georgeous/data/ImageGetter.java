...
public class ImageGetter implements ImageGetterWithManageSpace {
	...
	private static final long MAX_SIZE_OF_CACHE = 2000000; 
	
	/**
	 * Whether or not to load images from web when they are not available cached. Normally set to true, but can be
	 * set to false when caller uses ImageGetter to cache images (and so doesn't need to show them).
	 */
	...
	public ImageGetter(File externalCacheDir, Resources resources, DisplayMetrics metrics, boolean cachingOnly) throws IllegalStateException {
...
//				Log.v(TAG, "Hash for '" + filename + "' is: " + hash);
		...
		if (cachingOnly && !mExternalStorageWriteable) {
			throw new IllegalStateException("Trying to create ImageGetter for caching when external storage is unavailable.");
		}
	}
		...
		if (mExternalStorageAvailable && mExternalCacheDir != null) {
			...
			for (int i = 0; i < files.length; i++) {
				...
				if (cummulativeSize > MAX_SIZE_OF_CACHE) {
					...
					Log.v(TAG, "- " + (new Date(f.lastModified())).toString() + " (size: " + fileSize + ")");
					f.delete();
					cummulativeSize -= fileSize;
				}
			}
...
//					Log.v(TAG, "Second lookup in cache (after tryCaching()).");
...
//						Log.v(TAG, "Read image directly from URL.");
		...
		if (bmp != null) {
			setBounds(bmp);
			// TODO: save the bounds somewhere so we can invoke them when creating a blank image?
		}
		return bmp;
	}
		...
		try {
		    ...
		    try {
		        try {
		            ...
		            output.close();
//		            Log.v(TAG, "Created new cache file: " + file.getAbsolutePath());
		        }
		    } catch (Exception e) {
		        e.printStackTrace();
		        return false;
...
//			Log.v(TAG, "The file for url '" + url + "' should exist as: " + file.getAbsolutePath());
...
//			Log.v(TAG, "Found cached file for url '" + url + "' (" + filename + ")");
			...
			Log.v(TAG, "Did not find cached file for url '" + url + "' (hash: " + hash + ")");
			return null;
		}
	}
