...
public class FeedProvider extends ContentProvider {
	...
	static {
		sURIMatcher.addURI("net.filiph.georgeous.provider", "articles",
				ARTICLES);
		sURIMatcher.addURI("net.filiph.georgeous.provider", "articles/#",
				ARTICLES_ID);
	}
	...
	public static Uri getArticleByIdUri(long id) {
		return Uri.parse("content://" + AUTHORITY + "/" + TABLE_ARTICLES + "/"
				+ id);
	}
	...
	private Cursor getArticleById(long id) {
		...
		return qBuilder.query(mOpenHelper.getReadableDatabase(), null, null,
				null, null, null, null);
	}

	private Cursor getArticles(String[] projection, String selection,
			String[] selectionArgs, String sortOrder) {
		SQLiteQueryBuilder qBuilder = new SQLiteQueryBuilder();
		qBuilder.setTables(DbHelper.ARTICLE_TABLE_NAME);
		return qBuilder.query(mOpenHelper.getReadableDatabase(), projection,
